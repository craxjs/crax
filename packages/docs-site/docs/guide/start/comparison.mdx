...
- **File-based routing with loaders.** Crax's routing is file-based (like Next.js), but runs on the client. [Route loaders](../features/routing.mdx) let you fetch and preload data before render, similar to React Router's data loaders.
...
- **Image optimization.** The `<Image>` and `<Picture>` components handle responsive sizing, CDN-aware srcsets, and lazy loading. See [image optimization](../features/image-optimization.mdx) for details.
...
- **Global state management.** `createStore` is a lightweight, subscription-based state management tool with no external dependencies. It works with React's strict mode and includes history and locking utilities. See [state management](../features/state-management.mdx).
- **PWA and OG generation.** Crax scaffolds PWA manifest generation, icon resizing, and Open Graph image templates. See [PWA capabilities](../features/pwa.mdx).
