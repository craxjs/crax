...
Pages can export an async `loader` to fetch data before the route renders. See [Data Fetching](/crax/guide/features/data-fetching.md#route-level-data-loading) for the full pattern.

## getRoutes()

`getRoutes()` enumerates every discovered page route (`{ path, filePath, isDynamic }`), resolved fresh on each call — useful for SSG/prerender tooling that needs the full route list. See [Static Site Generation](/crax/guide/features/ssg.md) for an example.
